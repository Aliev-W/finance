        ...
        <div className="card space-y-3">
          ...
          {selectedWorker && (
            <div className="bg-blue-50 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-900 text-sm">{selectedWorker.name}</p>
                <p className="text-xs text-blue-600">{selectedWorker.position} · Oylik: {formatMoney(selectedWorker.salary_amount, selectedWorker.salary_currency)}</p>
              </div>
            </div>
          )}
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
              ...
              {existingPayments.map(p => (
                <p key={p.id} className="text-xs text-amber-600">
                  {p.payment_type === 'full' ? '✓' : '⏳'} {formatMoney(p.amount, p.currency)}
          ...
          {selectedWorker && hasExistingFull && (
            <div className="bg-green-50 border border-green-100 rounded-xl p-3 flex items-center gap-2">
              ...
              <p className="text-xs text-green-700 font-medium">Bu oy uchun to'liq oylik allaqachon to'langan</p>
            </div>
          )}
        </div>
