...
router.get('/excel', async (req, res) => {
  try {
    ...
    const allWorkers = await query('SELECT * FROM workers WHERE is_active = 1 ORDER BY name', []);
...
router.get('/print', async (req, res) => {
  try {
    ...
    const allWorkers = await query('SELECT * FROM workers WHERE is_active = 1 ORDER BY name', []);
    const payments = await query('SELECT p.* FROM payments p WHERE p.payment_month = ?', [month]);
...
<div class="np">
  ...
  <button class="pbtn gbtn" onclick="if(history.length>1){history.back()}else{window.close()}">← Orqaga</button>
</div>
