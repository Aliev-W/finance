...
router.get('/monthly', (req, res) => {
  ...
  const activeWorkers = query('SELECT * FROM workers WHERE is_active = 1 ORDER BY name', []);

  const paymentsThisMonth = query(
...
router.get('/annual', (req, res) => {
  ...
  const activeCount = query('SELECT count(*) as c FROM workers WHERE is_active = 1', [])[0]?.c || 0;

  const months = [];
  for (let m = 1; m <= 12; m++) {
    const month = `${year}-${String(m).padStart(2, '0')}`;
    const payments = query('SELECT worker_id, payment_type, amount, currency FROM payments WHERE payment_month = ?', [month]);
    const byW = {};
...
router.get('/worker/:id', (req, res) => {
  ...
  const payments = month
    ? query('SELECT * FROM payments WHERE worker_id = ? AND payment_month = ? ORDER BY paid_at DESC', [req.params.id, month])
    : query('SELECT * FROM payments WHERE worker_id = ? ORDER BY paid_at DESC', [req.params.id]);
